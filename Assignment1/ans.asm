...
# complete code written solely by me, MIPS learned from youtube playslist.
# LINK: https://www.youtube.com/watch?v=u5Foo6mmW0I&list=PL5b07qlmA3P6zUdDf-o97ddfpvPFuNa5A
...
main:
    ...
    # t2,t3 -> y1,y2
    # f12 -> saving the ans
    # f0,f2 -> temp registors for operations
    # f4 -> 2.0
...
posarea:
    ...
    mtc1.d $t0, $f0
    cvt.d.w $f0, $f0 # f0 --> (x2-x1)

    mtc1.d $t2, $f2
    cvt.d.w $f2, $f2 # f2 --> (y2+y1) 

    div.d $f0, $f0, $f4 # (x2-x1)/2

    mul.d $f0, $f0, $f2 # (x2-x1)*(y2+y1)/2
...
negarea:
    ...
    mtc1.d $t2, $f2
    cvt.d.w $f2, $f2 # f2 --> (y2^2+y1^2)

    div.d $f2, $f2, $f4 #(y2^2+y1^2)/2
    div.d $f2, $f2, $f0 #(y2^2+y1^2)/2*(y2-y1)

    mtc1.d $t0, $f0
    cvt.d.w $f0, $f0 # f0 --> (x2-x1)

    mul.d $f0, $f0, $f2 # (x2-x1)*(y2^2+y1^2)/2*(y2-y1)
